...
geo3 = l.Cylinder(r=r0, l=l2, lguard=float('inf'))

model1 = finite_radius_current
model2 = finite_length_current

Vs_geo1 = np.array([4]) # bias voltages
Vs_geo2 = np.array([2.5,4,5.5,10])
Vs_geo3 = np.array([4]) # bias voltages last one was supposed to be 7- electronics issue caused it to be 10V
...
if gendata == 1:
    synth_data=random_synthetic_data(N,geo1,geo2,geo3, model1,model2,Vs_geo1,Vs_geo2,Vs_geo3,geometry)
elif gendata == 0:
    synth_data=pd.read_csv('synth_data_mNLPs.csv',index_col=0)
else:
...
"""
Vs_all=np.concatenate((Vs_geo1,Vs_geo2,Vs_geo3))
...
for i, n, T, V0 in zip(count(), data['ne'], data['Te'], tqdm(data['V0'])):
    ...
    I_geo3[i] = model2(geo3, l.Electron(n=n, T=T), V=V0+Vs_geo3)
Itemp=np.append(I_geo1,I_geo2,axis=1)
I=np.append(Itemp,I_geo3,axis=1)

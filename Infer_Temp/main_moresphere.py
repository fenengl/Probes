...
Vs_geo1 = np.array([4,7]) # bias voltages
Vs_geo2 = np.array([2.5,5.5]) # bias voltages last one was supposed to be 7- electronics issue caused it to be 10V
...
Is_sph =np.array(synth_data.iloc[:,3])
Is_cyl =np.array(synth_data.iloc[:,4:])
...
if TF == 1:
    pred,results,history,net_model= tensorflow_network(Is_sph,Is_cyl,Ts,M)

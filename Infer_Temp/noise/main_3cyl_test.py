...
if gendata == 1:
    ...
    synth_data=pd.read_csv('../3_cyl_short/synth_data_cyl_0.csv',index_col=0)
else:
...
plt.title('e)')
plt.savefig('correlation_t.png', bbox_inches="tight")
...
plt.text(40,420,'RMSRE (%i - %i km) = ' %(range1,range2) + str(round(rms_rel_error(data_geo2['Te'].ravel()[range1:range2], predictions.ravel()[range1:range2]),3)))
...
debye = np.zeros(len(data_geo2['Te']))

def calc_eta(V,T):
    eta=(sc.elementary_charge*V)/(sc.Boltzmann*T)
    return eta

for i, nd, Td in zip(count(),data_geo2['ne'], data_geo2['Te']):
    debye[i]=l.Electron(n=nd, T=Td).debye

plt.figure()
plt.plot(debye, data_geo2['alt'], label='Debye')
plt.xlabel('Debye')
plt.ylabel('Altitude $[\mathrm{km}]$')
plt.legend()

plt.savefig('debye_t.png', bbox_inches="tight")


ratio=r0/debye
plt.figure()
plt.plot(ratio, data_geo2['alt'], label='Debye')
plt.xlabel('ratio r0/debye')
plt.ylabel('Altitude $[\mathrm{km}]$')
plt.legend()

plt.savefig('ratio_t.png', bbox_inches="tight")


plt.figure()
plt.plot(calc_eta(Vs_geo2[1],data_geo2['Te']), data_geo2['alt'], label='Debye')
plt.xlabel('eta')
plt.ylabel('Altitude $[\mathrm{km}]$')
plt.legend()

plt.savefig('eta_t.png', bbox_inches="tight")
...
print_table(
    [['l1'              , 'B1'              , 'V1'              ,'l2'              ,'B2'              , 'V2'              ,'l3'              ,'B3'               , 'V3'              ,'dB'              ,'sigma'              ,'RMSRE'
     , 'corr'                  ],
     [l1,np.mean(beta.Beta_cyl1),Vs_geo1[0],l2,np.mean(beta.Beta_cyl2),Vs_geo2[0],l2,np.mean(beta.Beta_cyl3),Vs_geo2[1],np.mean(beta.diff_Beta),sel_noise, rms_rel_error(data_geo2['Te'].ravel(), predictions.ravel()),pearsonr(data_geo2['Te'].ravel(),predictions.ravel())[0]]])
